...
const Community = ({ isOpen, onClose }) => {
  ...
  const handleSendRequest = async (user) => {
    try {
        const response = await axios.post("http://13.209.226.109:1111/friend/request", null, {
            params: {
                myId: userData.id,
  ...
  useEffect(() => {
    const fetchRequests = async() => {
      try {
        const response = await axios.get("http://13.209.226.109:1111/friend/request", {
          params: {
            myId: userData.id
          }
        });
  ...
  const handleRejectRequest = async (user) => {
    try {
      await axios.post("http://13.209.226.109:1111/friend/request/delete", null, {
        params: {
  ...
  const fetchChats = async() => {
    try {
      const url = "http://13.209.226.109:1111/" + userData.id + "/messageRooms";
