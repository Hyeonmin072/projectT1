...
public class WebSocketConfig implements WebSocketMessageBrokerConfigurer {
    ...
    public void registerStompEndpoints(StompEndpointRegistry registry) {
        ...
        registry.addEndpoint("/chat/inbox") // ws://localhost/chat/inbox를 호출하면 websocket 연결이 된다.
                .setAllowedOriginPatterns("http://3.34.143.32:80") // cors 허용을 위해 꼭 설정해주어야 한다.
    ...
    public void configureMessageBroker(MessageBrokerRegistry registry) {
        ...
        // publish
        registry.setApplicationDestinationPrefixes("/pub"); // 채팅 데이터를 전송할 때마다 /pub 관련 메서드를 호출해 채팅방을 구독하고 있는 유저에게 메시지
                                                            // 브로커가 메시지를 전달
